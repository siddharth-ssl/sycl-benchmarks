...
int main(int argc, char** argv)
{
  ...
  sycl::queue         q(sycl::default_selector_v, dpc_common::exception_handler, properties);
    ...
    std::size_t block_dim[3]  = {254,254,254};
    ...
    std::cout << "KIDA KE " << solver.ke()/(254*254*254) << std::endl;
    
    /// ----------------------------- ///
    /// @brief CPU IMPLEMENTATION
    /// ----------------------------- ///

    double collide_time(0.);
    auto start_cputime = std::chrono::high_resolution_clock::now();
    for(std::size_t it=1; it<=itr; it++)
    {
       collide_time = solver.time_step(m_dt, it, q);
    //   std::cout << "KIDA KE " << solver.ke()/(68*68*68) << std::endl;
    }
    ...
    std::cout << "Collide time per iteration " << collide_time << " Time elapsed per iteratiom " << duration_cputime.count()/(1e6*itr) << std::endl;
    return 0;
}
