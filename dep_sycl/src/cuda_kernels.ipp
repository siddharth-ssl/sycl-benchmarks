        ...
        sycl::queue& q)
{
    ...
    std::size_t cuda_tx = static_cast<std::size_t>(2);
    std::size_t cuda_ty = static_cast<std::size_t>(2);
    std::size_t cuda_tz = static_cast<std::size_t>(2);
    ...
    auto operations_sycl = [&](sycl::handler& h){
        sycl::stream out(100, 2, h);
        h.parallel_for(
            sycl::nd_range<3>(num_blocks * num_threads, num_threads),
            [=](sycl::nd_item<3> item_ct) 
            {
                operations_device(npx, npy, xmin, xmax, ymin, ymax, zmin, zmax, device_data_1, device_data_2, item_ct, out);
